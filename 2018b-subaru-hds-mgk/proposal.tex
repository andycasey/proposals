...
\backup{In separate work, we have applied the same search technique to identify lithium-rich giant stars from LAMOST data. In the case of inclement weather, we will observe bright ($V < 10$) lithium-rich giant stars selected from LAMOST, with a lithium-normal giant of identical stellar parameters for every lithium-rich giant, in order to perform the first ever differential analysis of lithium-rich giant stars. Our sample of lithium-rich giant stars are bright and span across all right ascensions, making them very suitable backup targets. The proposed setup (SETUP-Ra) for Mg/K stars is suitable for our lithium-rich giant analysis.}
